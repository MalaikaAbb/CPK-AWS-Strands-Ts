...
[Shared state](/strands-typescript/shared-state) is most powerful when the agent's state shows up in
...
transition, or [streamed update](/strands-typescript/shared-state/streaming), `useAgent` re-renders this component
...
This is the same two-way channel described in [Shared State](/strands-typescript/shared-state). The
...
- [Shared State](/strands-typescript/shared-state): the full read/write model and the underlying `useAgent` subscription.
- [State streaming](/strands-typescript/shared-state/streaming): stream a tool argument into a state key so the canvas fills in token-by-token.
- [Agent read-only context](/strands-typescript/shared-state/agent-readonly): push UI values *to* the agent without letting it write back.
