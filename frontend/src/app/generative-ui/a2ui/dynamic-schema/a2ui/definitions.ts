/**
 * The dynamic-schema catalog's component definitions, as published.
 *
 * This is the one A2UI file the doc page prints end to end — it opens with its
 * imports and closes with `} satisfies CatalogDefinitions;`. Copied verbatim,
 * including the two long inline notes (the B12 row-keys constraint on
 * `DataTable`, and why `PrimaryButton.action` is `z.unknown()` rather than
 * `z.any()`).
 *
 * One line is added at the bottom: `export type MyDefinitions`. The renderers
 * file the page shows next is typed `CatalogRenderers<MyDefinitions>`, and that
 * type is exported from nowhere — see the doc gaps on this route.
 *
 * The component names here must match the `A2UI_DYNAMIC_COMPOSITION_RULES`
 * string in `backend/src/agents/a2ui-dynamic-agent.ts`, which is what teaches
 * the secondary planner which components it may emit.
 */
...
/** Not published. Named by `renderers.tsx` and exported by no doc snippet. */
