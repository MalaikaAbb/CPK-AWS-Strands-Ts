 ...
 *
 * Carried over from the Google ADK harness, whose A2UI routes hit the same
 * omission. The dynamic-schema route here has its own `a2ui/primitives.tsx`
 * with a different set (CardShell, chart colours, the `c` tokens); this file
 * serves the fixed-schema route, which needs plain `Card` and `Separator`
 * instead. Two files rather than one because the two catalogs genuinely need
 * different primitives, and merging them would couple two independent routes.
 *
