...
export const renderers: CatalogRenderers<Definitions> = {
  ...
  /**
   * Button override: pure presentation. The schema declares an `action` with a
   * `book_flight` event for fidelity, but nothing handles it — the published
   * `display_flight` returns a bare operations envelope and registers no
   * handler for the event. The doc page discusses `action_handlers` only in
   * terms of the Python SDK; no TypeScript equivalent is shown anywhere. See
   * the route page.
   */
