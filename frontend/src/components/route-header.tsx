...
import { DocGaps } from "@/components/doc-gaps";
import {
  STATUS_LABEL,
  demoPath,
  docUrl,
...
export function RouteHeader({ path }: { path: string }) {
  ...
  return (
    ...
    {/* Deliberately above the route's own prose: several of these routes chat
        happily while the feature the doc page teaches does not exist. */}
    <DocGaps path={path} />
    </>
  );
}
